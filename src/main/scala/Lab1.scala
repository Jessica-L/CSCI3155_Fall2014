object Lab1 extends jsy.util.JsyApplication {
  ...
  // returns the absolute value by changing the sign if it is less than zero 
  ...
  // returns n times the input string and a blank string if n = 0
  ...
  // calculates one iteration of Newton's approximation with tangent lines.
  def sqrtStep(c: Double, xn: Double): Double = { xn - ((xn * xn - c)/ (2*xn)) }
  
  // recusavily calls sqrtStep n times
  ...
  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double = {
	...
	val x = sqrtStep(c, x0)
	if (abs((x*x)-c) < epsilon ) x
	else sqrtErr(c,x,epsilon)
}
  ...
  def eval(e: Expr): Double = e match {
    case N(n) => n
    case Unary(Neg, n) => ((-1)* eval(n))
    case Binary(Plus, n, m) => (eval(n) + eval(m))
    case Binary(Minus, n, m) => (eval(n) - eval(m))
    case Binary(Times, n, m) => (eval(n) * eval(m))
    case Binary(Div, n, m) => (eval(n) / eval(m))
